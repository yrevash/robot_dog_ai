...
class GestureClassifier:
    ...
    def _classify(self, lmk, hand_label: str) -> Optional[str]:
        ...
        # Index lean (for LEFT/RIGHT): tip-to-MCP dx normalised by palm width.
        # palm_w = |index_MCP.x - pinky_MCP.x| gives a hand-size-invariant scale.
        # dx5_norm < -0.07 → finger tips left of its own MCP (LEFT)
        # dx5_norm > -0.07 → finger tips right of or at its MCP (RIGHT)
        _palm_w = abs(float(lmk[5].x - lmk[17].x)) + 1e-6
        idx_dx_norm = (float(lmk[8].x) - float(lmk[5].x)) / _palm_w
